...
const EarnedBonusPoints = ({}) => {
  ...
  const userID = async userId => {
    try {
      const response = await axios.get(
        `https://justbuynewbackend.onrender.com/api/v1/user/auth/user/${userId}`,
      );
  ...
  const fetchUserReferralStats = async userId => {
    try {
      const response = await axios.get(
        `https://justbuynewbackend.onrender.com/api/v1/user/${userId}`,
      );
      if (response.status === 200) {
  ...
  const walletHistoryID = async userId => {
    ...
    try {
      const response = await axios.get(
        `https://justbuynewbackend.onrender.com/api/v1/wallet-history/${userId}`,
      );
      if (response.status === 200) {
